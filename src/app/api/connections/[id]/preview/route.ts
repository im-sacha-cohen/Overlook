...
const KINDS: WriteOp["kind"][] = ["updateRows", "deleteRows", "addColumn", "renameColumn", "changeColumnType", "dropColumn", "dropTables", "emptyTables"];
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if ((op.kind === "dropTables" || op.kind === "emptyTables") && !Array.isArray(op.tables)) return errorResponse(new Error("tables doit être une liste"));
